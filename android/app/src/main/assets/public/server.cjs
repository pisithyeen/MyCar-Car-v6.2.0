...
var activeProfile = simulatedUsersDatabase[0];
var defaultWeaknessReport = {
  commonIssues: [
...
var notificationLogsDatabase = [
  {
    ...
    channel: "Push",
    status: "unread",
    sentAt: new Date(Date.now() - 3 * 36e5).toISOString()
    // 3 hours ago
  },
  {
    id: "notif-2",
    vehicleId: "v1",
    reminderId: "rem-2",
    title: "\u{1F514} Upcoming Service: Front Brake Pads Inspection",
    ...
    channel: "In-App",
    status: "unread",
    sentAt: new Date(Date.now() - 1 * 36e5).toISOString()
    // 1 hour ago
  }
];
var forumPostsDatabase = [
  {
    id: "fp-1",
    ...
    comments: [
      ...
      },
      {
        ...
        timestamp: new Date(Date.now() - 3.5 * 36e5).toISOString(),
        upvotes: 5,
    ...
    comments: [
      {
        ...
        timestamp: new Date(Date.now() - 20 * 36e5).toISOString(),
        upvotes: 15,
...
var classifiedListingsDatabase = [
  {
    id: "part-1",
    ...
    comments: [
      {
        ...
        createdAt: new Date(Date.now() - 2 * 8644e4).toISOString()
      },
      {
        id: "p-comm-2",
        listingId: "part-1",
        ...
        createdAt: new Date(Date.now() - 1.5 * 8644e4).toISOString()
      }
    ],
    ...
    views: 42,
    offerCount: 1,
    createdAt: new Date(Date.now() - 3.5 * 8644e4).toISOString(),
    isBoosted: true,
  ...
  },
  {
    ...
    views: 128,
    offerCount: 0,
    createdAt: new Date(Date.now() - 1 * 8644e4).toISOString(),
    isBoosted: false,
  ...
  },
  {
    ...
    views: 89,
    offerCount: 0,
    createdAt: new Date(Date.now() - 5 * 8644e4).toISOString(),
    isBoosted: false,
  ...
  },
  {
    ...
    views: 19,
    offerCount: 1,
    createdAt: new Date(Date.now() - 7 * 8644e4).toISOString(),
    isBoosted: false,
...
var partOffersDatabase = [
  {
    ...
    status: "Pending",
    createdAt: new Date(Date.now() - 2.5 * 36e5).toISOString()
  }
];
var partReportsDatabase = [];
var garagesDatabase = [
  {
    id: "g1",
...
async function generateContentWithRetry(client, params, maxRetries = 2) {
  ...
  for (const model of modelsToTry) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        lastError = err;
        console.warn(`[MyCar Care KH AI] Attempt failed with model ${model}:`, err.message || err);
        if (model === modelsToTry[modelsToTry.length - 1] && attempt === maxRetries) {
          break;
        }
        const delay = Math.min(1e3 * Math.pow(2, attempt) + Math.random() * 500, 4e3);
        await new Promise((resolve) => setTimeout(resolve, delay));
      }
    }
  }
  throw lastError || new Error("Failed to query Gemini after model retry list exhaustion.");
}
function calculateReminders(vehicle, records) {
  const list = [];
  ...
  const getDaysSince = (dateStr) => {
    ...
    const diffDays = Math.floor(diffMs / (1e3 * 3600 * 24));
    return diffDays >= 0 ? diffDays : 0;
  };
  const getLatestRecord = (categoryKey) => {
    return records.filter((r) => r.vehicleId === vehicle.id && r.serviceCategory.toLowerCase().includes(categoryKey.toLowerCase())).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0];
  };
  if (fuel !== "ev" && fuel !== "electric") {
    ...
    if (oilRecord) {
      ...
      diffOilDays = vehicle.lastServiceDate ? getDaysSince(vehicle.lastServiceDate) : 120;
    }
    if (diffOilKm >= 5e3 || diffOilDays >= 90) {
      list.push({
        id: `r-oil-${vehicle.id}`,
    ...
    } else if (diffOilKm >= 4e3 || diffOilDays >= 75) {
      list.push({
        id: `r-oil-${vehicle.id}`,
        vehicleId: vehicle.id,
        service: "Engine Oil Service",
  ...
  if (brakeRecord) {
    ...
    diffBrakeKm = Math.min(kmNow, 22e3);
    diffBrakeDays = 210;
  }
  if (diffBrakeKm >= 2e4 || diffBrakeDays >= 180) {
    list.push({
  ...
  } else if (diffBrakeKm >= 15e3 || diffBrakeDays >= 150) {
    list.push({
      id: `r-brake-${vehicle.id}`,
      vehicleId: vehicle.id,
      service: "Brake Service",
  ...
  if (tireRecord) {
    ...
    diffTireKm = Math.min(kmNow, 12e3);
    diffTireDays = 190;
  }
  if (diffTireKm >= 1e4 || diffTireDays >= 180) {
    list.push({
  ...
  } else if (diffTireKm >= 8e3 || diffTireDays >= 150) {
    list.push({
      id: `r-tire-${vehicle.id}`,
      vehicleId: vehicle.id,
      service: "Tire Service",
  ...
  if (fuel === "ev" || fuel === "electric") {
    ...
    if (evRecord) {
      ...
      diffEvKm = Math.min(kmNow, 22e3);
      diffEvDays = 370;
    }
    if (diffEvKm >= 2e4 || diffEvDays >= 365) {
      list.push({
        id: `r-ev-${vehicle.id}`,
        vehicleId: vehicle.id,
    ...
    if (diffEvCoolantKm >= 8e4 || diffEvCoolantDays >= 730) {
      list.push({
    ...
    } else if (diffEvCoolantKm >= 65e3 || diffEvCoolantDays >= 600) {
      list.push({
        id: `r-ev-coolant-${vehicle.id}`,
        vehicleId: vehicle.id,
        service: "EV Coolant Flush",
  ...
  if (fuel === "hybrid") {
    ...
    if (hybridRecord) {
      ...
      diffHyKm = Math.min(kmNow, 16e3);
      diffHyDays = 195;
    }
    if (diffHyKm >= 15e3 || diffHyDays >= 180) {
      list.push({
    ...
    } else if (diffHyKm >= 12e3 || diffHyDays >= 150) {
      list.push({
        id: `r-hybrid-${vehicle.id}`,
        vehicleId: vehicle.id,
        service: "Hybrid System Check",
  ...
  if (fuel === "gasoline" || fuel === "petrol") {
    ...
    if (diffSparkKm >= 4e4 || diffSparkDays >= 720) {
      list.push({
  ...
  if (fuel === "diesel") {
    ...
    if (diffDFilterKm >= 15e3 || diffDFilterDays >= 180) {
      list.push({
  ...
  if (kmNow >= 1e5 && fuel !== "ev" && fuel !== "electric") {
  ...
  if (kmNow >= 6e4) {
    ...
    let diffTransDays = transRecord ? getDaysSince(transRecord.date) : 1e3;
    if (diffTransKm >= 6e4 || diffTransDays >= 1e3) {
      list.push({
...
var partnerAccessRequestsDatabase = [
  {
    id: "req-1",
    vehicleId: "v2",
    ...
    requestedAt: new Date(Date.now() - 3600 * 1e3).toISOString(),
    status: "pending"
  }
];
var vehiclePartnerPermissionsDatabase = [
  {
    id: "perm-1",
    vehicleId: "v1",
    garageId: "g1",
    garageName: "Apsara Auto Repair & Diagnostic Center",
    grantedAt: new Date(Date.now() - 86400 * 1e3).toISOString(),
    status: "trusted"
  }
];
...
var monthlyMaintenanceDatabase = [
  {
    id: "m-month-v1",
    vehicleId: "v1",
    ownerId: "yeonpisith",
...
app.put("/api/privacy-settings", (req, res) => {
  ...
  if (showFullName !== void 0) ownerPrivacySettings.showFullName = showFullName;
  if (showPhone !== void 0) ownerPrivacySettings.showPhone = showPhone;
  if (showPlateNumber !== void 0) ownerPrivacySettings.showPlateNumber = showPlateNumber;
  if (showPreviousHistory !== void 0) ownerPrivacySettings.showPreviousHistory = showPreviousHistory;
  if (allowFollowUps !== void 0) ownerPrivacySettings.allowFollowUps = allowFollowUps;
  res.json(ownerPrivacySettings);
});
...
app.post("/api/qr/scan", (req, res) => {
  ...
  if (found.type === "user") {
    res.json({
      ...
      vehicles: vehicles.map((v) => {
        ...
        return {
          ...
          previousServiceHistory: isPermitted && ownerPrivacySettings.showPreviousHistory ? maintenanceRecords.filter((m) => m.vehicleId === v.id) : maintenanceRecords.filter((m) => m.vehicleId === v.id && m.provider === scannedByGarageName)
        };
      })
    });
  } else {
    ...
    res.json({
      ...
      permissionRequired: !isPermitted,
      previousServiceHistory: isPermitted && ownerPrivacySettings.showPreviousHistory ? maintenanceRecords.filter((m) => m.vehicleId === car.id) : maintenanceRecords.filter((m) => m.vehicleId === car.id && m.provider === scannedByGarageName)
    });
  }
});
app.post("/api/qr/manual-lookup", (req, res) => {
  ...
  let foundToken = "";
  const matchedCar = vehicles.find(
    (v) => v.id.toUpperCase() === normalized || v.publicVehicleId?.toUpperCase() === normalized || v.publicVehicleId && v.publicVehicleId.replace(/-/g, "").toUpperCase() === normalized.replace(/-/g, "")
  );
  if (matchedCar) {
...
app.get("/api/owner/partner-requests", (req, res) => {
  res.json(partnerAccessRequestsDatabase);
});
app.post("/api/owner/partner-requests/:id/approve", (req, res) => {
  const { id } = req.params;
...
app.post("/api/owner/partner-requests/:id/reject", (req, res) => {
  const { id } = req.params;
  const foundReq = partnerAccessRequestsDatabase.find((r) => r.id === id);
  if (!foundReq) return res.status(404).json({ error: "Access request not found." });
...
app.get("/api/owner/partner-permissions", (req, res) => {
  res.json(vehiclePartnerPermissionsDatabase.filter((p) => p.status !== "revoked"));
});
app.post("/api/owner/partner-permissions/:id/revoke", (req, res) => {
  const { id } = req.params;
  ...
  notificationLogsDatabase.unshift({
    ...
    title: `\u{1F512} Partner Access Revoked`,
...
app.post("/api/garage/request-access", (req, res) => {
  ...
  notificationLogsDatabase.unshift({
    ...
    vehicleId,
    title: `\u{1F511} Portal Access Request`,
...
app.post("/api/vehicles/regenerate-qr/:id", (req, res) => {
  const { id } = req.params;
...
app.get("/api/vehicles/:id/scan-logs", (req, res) => {
  const { id } = req.params;
  const car = vehicles.find((v) => v.id === id);
  if (!car) return res.status(404).json({ error: "Vehicle not found." });
  const records = scanLogs.filter((log) => {
    return log.targetType === "vehicle" && log.targetId === car.id || log.token && (log.token === car.qrSecureToken || log.token === `MCC-CAR-${car.id.toUpperCase()}`);
  });
  res.json(records);
});
app.post("/api/garage/service-records", (req, res) => {
  const {
    vehicleId,
    vehicleLabel,
    serviceCategory,
  ...
  const newRecord = {
    ...
    serviceCategory,
    serviceDate: serviceDate || (/* @__PURE__ */ new Date()).toISOString().split("T")[0],
  ...
  notificationLogsDatabase.unshift({
    ...
    vehicleId,
    title: `\u{1F527} Service Approval Requested`,
...
app.get("/api/owner/pending-records", (req, res) => {
  res.json(garageServiceRecords);
});
app.post("/api/owner/service-records/:id/approve", (req, res) => {
  const { id } = req.params;
  ...
  if (record.nextServiceDate || record.nextServiceMileage) {
    ...
    const newRem = {
      ...
      priority: "Medium",
      dueDate: record.nextServiceDate || void 0,
      dueMileage: record.nextServiceMileage || void 0,
      isAiSuggested: false,
      createdAt: (/* @__PURE__ */ new Date()).toISOString()
    };
...
app.post("/api/owner/service-records/:id/reject", (req, res) => {
  const { id } = req.params;
  const idx = garageServiceRecords.findIndex((r) => r.id === id);
...
app.post("/api/owner/service-records/:id/dispute", (req, res) => {
  const { id } = req.params;
...
app.post("/api/owner/service-records/:id/request-correction", (req, res) => {
  const { id } = req.params;
...
app.get("/api/owner/appointments", (req, res) => {
  res.json(appointmentsDatabase);
});
app.post("/api/owner/appointments", (req, res) => {
  ...
  notificationLogsDatabase.unshift({
    ...
    vehicleId,
    title: `\u{1F4C5} Appointment Booked`,
...
app.put("/api/owner/appointments/:id", (req, res) => {
  const { id } = req.params;
...
app.delete("/api/owner/appointments/:id", (req, res) => {
  const { id } = req.params;
  appointmentsDatabase = appointmentsDatabase.filter((a) => a.id !== id);
  res.json({ success: true });
});
app.get("/api/owner/monthly-maintenance", (req, res) => {
  res.json(monthlyMaintenanceDatabase);
});
app.post("/api/owner/monthly-maintenance", (req, res) => {
...
app.put("/api/owner/monthly-maintenance/:id", (req, res) => {
  const { id } = req.params;
...
app.post("/api/owner/monthly-maintenance/:id/toggle-item", (req, res) => {
  const { id } = req.params;
  const { itemId, status, checkedBy } = req.body;
  const idx = monthlyMaintenanceDatabase.findIndex((m) => m.id === id);
  if (idx === -1) return res.status(404).json({ error: "Monthly checklist not found." });
  const checklist = monthlyMaintenanceDatabase[idx];
  const itemIdx = checklist.checklistItems.findIndex((it) => it.id === itemId);
  if (itemIdx > -1) {
    checklist.checklistItems[itemIdx].status = status;
    checklist.checklistItems[itemIdx].checkedBy = checkedBy;
    checklist.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
  }
  res.json(checklist);
});
app.get("/api/owner/garage-connections", (req, res) => {
  res.json(connectionsDatabase);
});
app.post("/api/owner/garage-connections", (req, res) => {
  ...
  const newConn = {
    ...
    allowCreateLogs: allowCreateLogs !== void 0 ? allowCreateLogs : true,
    allowSendAppointments: allowSendAppointments !== void 0 ? allowSendAppointments : true,
    allowTelegramReminders: allowTelegramReminders !== void 0 ? allowTelegramReminders : false,
    allowInvoiceUpload: allowInvoiceUpload !== void 0 ? allowInvoiceUpload : true,
    status: "connected",
...
app.put("/api/owner/garage-connections/:id", (req, res) => {
  const { id } = req.params;
...
app.delete("/api/owner/garage-connections/:id", (req, res) => {
  const { id } = req.params;
...
app.get("/api/admin/all-service-records", (req, res) => {
  res.json(garageServiceRecords);
});
app.post("/api/admin/service-records/:id/lock", (req, res) => {
  const { id } = req.params;
  const idx = garageServiceRecords.findIndex((r) => r.id === id);
  if (idx === -1) return res.status(404).json({ error: "Record not found." });
...
app.post("/api/garage/follow-up", (req, res) => {
  ...
  notificationLogsDatabase.unshift({
    ...
    vehicleId,
    title: `\u{1F514} Follow-Up Alert from ${garageName || "Apsara Auto"}`,
...
app.put("/api/profile", (req, res) => {
  ...
  let user = simulatedUsersDatabase.find(
  ...
  if (!user) {
    ...
    user = {
      ...
      licenseNumber: licenseNumber || void 0,
      activatedModules: activatedModules || void 0,
      isMultiService: isMultiService || void 0
    };
    simulatedUsersDatabase.push(user);
  } else {
    user.name = name;
    user.phone = phone;
    ...
    user.activatedModules = activatedModules;
    user.isMultiService = isMultiService;
  }
  if (role === "Admin" && email !== "admin@mycar.com.kh") {
...
app.put("/api/admin/users/:id", (req, res) => {
  ...
  simulatedUsersDatabase[idx] = {
    ...
    businessName: businessName !== void 0 ? businessName : oldUser.businessName,
    licenseNumber: licenseNumber !== void 0 ? licenseNumber : oldUser.licenseNumber
  };
  if (activeProfile.id === Number(id)) {
...
app.post("/api/admin/users/:id/reset-password", (req, res) => {
  ...
  res.json({ success: true, resetToken: `reset-${Math.floor(1e5 + Math.random() * 9e5)}` });
});
app.post("/api/admin/users/:id/send-notif", (req, res) => {
  const { id } = req.params;
  ...
  notificationLogsDatabase.unshift({
    ...
    title: "\u{1F4E2} Official Channel Communication from Head Office",
...
app.post("/api/admin/transfer-vehicle", (req, res) => {
...
app.post("/api/vehicles", async (req, res) => {
  ...
  const newVehicle = {
    ...
    lastServiceDate: lastServiceDate || (/* @__PURE__ */ new Date()).toISOString().split("T")[0],
    nickname: nickname || "",
...
Keep issues highly relevant to brand/model/year. Make sure strings of Cambodia details exist. Check for local parts sourcing.`;
              ...
              commonIssues: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    risk: { type: import_genai.Type.STRING, enum: ["low", "medium", "high"] }
                  },
                  required: ["issue", "advice", "risk"]
                }
              },
              maintenancePriority: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
              strongPoints: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
              weakPoints: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
              monthlyChecklist: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
              longTripChecklist: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
              recommendedSchedule: {
                type: import_genai.Type.ARRAY,
                items: {
                  type: import_genai.Type.OBJECT,
                  properties: {
                    ...
                    interval: { type: import_genai.Type.STRING }
                  },
                  required: ["task", "interval"]
                }
              },
              warningSigns: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } }
            },
            required: [
              "commonIssues",
              "maintenancePriority",
      ...
      console.error("Gemini failed to generate report, supplying high quality simulated report", e);
...
app.put("/api/vehicles/:id", (req, res) => {
  ...
  if (nickname !== void 0) vehicle.nickname = nickname;
  if (plateNumber !== void 0) vehicle.plateNumber = plateNumber;
  if (vehicleType !== void 0) vehicle.vehicleType = vehicleType;
  if (purchaseDate !== void 0) vehicle.purchaseDate = purchaseDate;
  if (purchasePrice !== void 0) vehicle.purchasePrice = purchasePrice ? Number(purchasePrice) : void 0;
  if (photoUrl !== void 0) vehicle.photoUrl = photoUrl;
  if (notes !== void 0) vehicle.notes = notes;
  if (lastOilChangeMileage !== void 0) vehicle.lastOilChangeMileage = lastOilChangeMileage ? Number(lastOilChangeMileage) : void 0;
  if (lastServiceDate !== void 0) vehicle.lastServiceDate = lastServiceDate;
  res.json(vehicle);
});
...
app.post("/api/expenses", (req, res) => {
  ...
  const newExpense = {
    ...
    paymentMethod,
    receiptImage: receiptImage || void 0,
    notes: notes || ""
  };
...
app.post("/api/ai/driver-insights", async (req, res) => {
...
Respond in strict JSON with matching lower camel-case keys. Pure JSON only:
{
  ...
  "costPrediction": "string"
      ...
      const response = await generateContentWithRetry(client, {
        ...
        config: {
          ...
          responseSchema: {
            type: import_genai.Type.OBJECT,
            properties: {
              maintenanceInsights: { type: import_genai.Type.STRING },
              spendingInsights: { type: import_genai.Type.STRING },
              warningMessages: { type: import_genai.Type.STRING },
              ownershipAdvice: { type: import_genai.Type.STRING },
              costPrediction: { type: import_genai.Type.STRING }
            },
      ...
      console.error("Gemini failed insights generation, falling back to schema simulation", err);
    }
  }
...
app.get("/api/reminders/:vehicleId", (req, res) => {
  ...
  const calculatedReminders = calculated.map((c) => {
    let cat = "Custom Reminder";
  ...
  const evaluatedStored = storedReminders.map((r) => {
    ...
    if ((r.reminderType === "date_based" || r.reminderType === "date_and_mileage") && r.dueDate) {
      ...
      const diffDays = Math.ceil(diffMs / (1e3 * 3600 * 24));
      if (diffDays < 0) {
        worstStatus = "Overdue";
      } else if (diffDays === 0) {
  ...
  combined.forEach((rem) => {
    if (rem.status === "Overdue" || rem.status === "Due soon" || rem.status === "Due today") {
      ...
      if (!alreadyLogged) {
        notificationLogsDatabase.unshift({
          id: `notif-auto-${Date.now()}-${Math.floor(Math.random() * 1e3)}`,
          vehicleId,
          reminderId: rem.id,
          title: rem.status === "Overdue" ? `\u{1F6A8} CRITICAL: ${rem.service} is Overdue` : `\u26A0\uFE0F NOTICE: ${rem.service} is Due soon`,
          message: rem.reason,
          channel: "In-App",
          status: "unread",
          sentAt: (/* @__PURE__ */ new Date()).toISOString(),
...
app.post("/api/reminders", (req, res) => {
  ...
  const newReminder = {
    ...
    dueMileage: dueMileage ? Number(dueMileage) : void 0,
    repeatType: repeatType || "none",
    notificationTime: notificationTime || void 0,
  ...
  notificationLogsDatabase.unshift({
    ...
    vehicleId,
    reminderId: id,
    title: `\u{1F514} Scheduled Reminder: ${title}`,
    message: `New reminder set successfully for ${vLabel}. Trigger criteria: ${reminderType === "date_based" ? `Date (${dueDate})` : reminderType === "mileage_based" ? `Odometer (${dueMileage} km)` : `Criteria reached.`}`,
    channel: "In-App",
    status: "unread",
    sentAt: (/* @__PURE__ */ new Date()).toISOString()
  });
...
app.post("/api/reminders/:id/complete", (req, res) => {
  ...
  if (index !== -1) {
    ...
    original.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
    if (original.repeatType && original.repeatType !== "none") {
      ...
      if (original.dueMileage) {
        nextMileage = original.dueMileage + 5e3;
      }
      const nextReminder = {
        ...original,
        id: nextId,
...
app.post("/api/ai/reminder-suggestion", async (req, res) => {
  const { vehicleId } = req.body;
  const vehicle = vehicles.find((v) => v.id === vehicleId);
  if (!vehicle) {
...
`;
  ...
  try {
    ...
    console.error("Gemini suggestion failed, supplying mock diagnostic report:", err.message);
    ...
    const sampleFallbacks = [
      {
        ...
        description: isEV ? `Verify high voltage EV battery hermetic sealing before Phnom Penh high monsoon street accumulation.` : `Monsoon humidity degrades lubricating indices. Standard ${vehicle.brand} ${vehicle.model} models driving in Cambodia benefit from engine oil swaps every 5,000 km.`,
        reminderType: "date_and_mileage",
        priority: "High",
        dueDate: new Date(Date.now() + 45 * 24 * 3600 * 1e3).toISOString().split("T")[0],
        dueMileage: vehicle.mileage + 5e3,
      ...
      },
      {
        ...
        reminderType: "mileage_based",
        priority: "Medium",
        dueDate: new Date(Date.now() + 60 * 24 * 3600 * 1e3).toISOString().split("T")[0],
      ...
      },
      {
        ...
        reminderType: "date_based",
        priority: "High",
        dueDate: new Date(Date.now() + 30 * 24 * 3600 * 1e3).toISOString().split("T")[0],
        dueMileage: vehicle.mileage + 4e3,
...
app.post("/api/ai/partners/bilingual-ticket", async (req, res) => {
    ...
    const response = await generateContentWithRetry(client, {
      ...
      config: {
        ...
        responseSchema: {
          type: import_genai.Type.OBJECT,
          properties: {
            khmerNote: { type: import_genai.Type.STRING },
            englishNote: { type: import_genai.Type.STRING },
            bilingualCombined: { type: import_genai.Type.STRING },
            recommendedCheckLimit: { type: import_genai.Type.STRING }
          },
...
var notificationTemplatesDatabase = [
  {
    id: "templ-1",
...
var userNotificationSettingsDatabase = {
  pushEnabled: true,
  emailEnabled: true,
  smsEnabled: false,
...
var suspendedGaragesDatabase = {
  "cheater": true
  // Seeded scam garage suspended immediately
};
var garageMessageTemplatesDatabase = [
  {
    id: "g_tmpl_1",
    garageId: "g1",
  ...
  },
  {
    ...
    templateType: "pickup",
    messageBody: "\u{1F3C1} Good news {customer}! Your {vehicle} has completed servicing and passed all safety checks. It is ready for pickup at {garage}. Total Invoice: ${cost} USD. See you soon!",
    isActive: true,
    createdByStaffId: "g1_owner",
  ...
  },
  {
    ...
    messageBody: "\u{1F9FE} Tax Invoice {invoiceId} for ${cost} USD has been finalized for your {vehicle} at {garage}. Payment is due within 48h. Thank you!",
    isActive: true,
    createdByStaffId: "g1_owner",
  ...
  },
  {
    ...
    messageBody: "\u{1F4C5} Next Scheduled Check-up: Your {vehicle} is due for a regular check-up (tires/brakes). Securing your bay early at {garage} ensures zero wait time!",
    isActive: true,
    createdByStaffId: "g1_owner",
  ...
  },
  {
    ...
    templateType: "reminder",
    messageBody: "\u{1F4E6} Spare Parts Ordered: The parts package for your {vehicle} ({partName}) has been requested by {garage}. Transiting from seaport hub.",
    isActive: true,
    createdByStaffId: "g1_owner",
  ...
  },
  {
    ...
    templateType: "reminder",
    messageBody: "\u{1F4E6} Spare Parts Arrived: The parts ordered for your {vehicle} ({partName}) have arrived at {garage}. Ready for installation!",
    isActive: true,
    createdByStaffId: "g1_owner",
  ...
  },
  {
    ...
    templateType: "reminder",
    messageBody: "\u{1F6E1}\uFE0F Warranty Registered: Your recent repair {partName} completed at {garage} is covered under a parts warranty for 6 months.",
    isActive: true,
    createdByStaffId: "g1_owner",
  ...
  },
  {
    ...
    templateType: "reminder",
    messageBody: "\u{1F6E1}\uFE0F Warranty Alert: The warranty on your {partName} from {garage} is ending soon (within 30 days). Let us know if you need any inspection!",
    isActive: true,
    createdByStaffId: "g1_owner",
...
var telegramConnectionsDatabase = [
  {
    id: "tg_conn_1",
    userId: "1",
...
var garageCustomerNotificationPermissionsDatabase = [
  {
    id: "gcnp_1",
    garageId: "g1",
    garageName: "Apsara Auto Repair & Diagnostic Center",
...
var customTelegramMessagesSentCount = {};
...
app.post("/api/telegram/generate-token", (req, res) => {
  const token = `KH-${Math.floor(1e3 + Math.random() * 9e3)}`;
  userNotificationSettingsDatabase.verificationToken = token;
  res.json({ token });
});
app.post("/api/telegram/toggle-block-garage", (req, res) => {
  ...
  if (perm) {
    ...
    perm.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
...
app.post("/api/telegram/report-spam", (req, res) => {
  const { garageId } = req.body;
  const perm = garageCustomerNotificationPermissionsDatabase.find((p) => p.garageId === garageId);
  if (perm) {
...
app.post("/api/telegram/update-garage-permissions", (req, res) => {
...
app.post("/api/telegram/simulate-bot-command", (req, res) => {
  const { text } = req.body;
  ...
  if (formattedCommand === "/start") {
    if (tokenArg) {
      if (tokenArg === userNotificationSettingsDatabase.verificationToken || tokenArg === "KH-9901") {
        ...
        userNotificationSettingsDatabase.telegramConnectedAt = (/* @__PURE__ */ new Date()).toISOString();
        ...
        replyText = `\u{1F389} MyCar Care KH Bot connected successfully!
...
Please generate a fresh verification token inside App Settings > Connect Telegram and retry with /start [token].`;
      ...
      replyText = `\u{1F1F0}\u{1F1ED} MyCar Care KH Bot
...
\u2022 Status: Active & Connected\u{1F7E2}
...
Type /off to temporarily pause Telegram logs.`;
      ...
      replyText = `\u26A0 Bot Connection Status: Disconnected\u{1F534}
...
Please connect your profile via Settings first.`;
    ...
    replyText = `\u{1F514} MyCar Care KH Notifications: Enabled.
...
You will receive vehicle alarms, invoices, and service records in this chat.`;
    ...
    replyText = `\u{1F515} MyCar Care KH Notifications: Disabled (Silent mode active).
...
Preferences saved. Reactivate anytime by typing /on.`;
    ...
    replyText = `\u{1F5D1} Pairing Removed successfully. Your Telegram Chat ID has been wiped from our system records. Connection status is now offline.`;
  } else if (formattedCommand === "/help") {
    replyText = `\u{1F4A1} Available Commands List:
...
app.post("/api/telegram/garage-send", (req, res) => {
  ...
  const perm = garageCustomerNotificationPermissionsDatabase.find((p) => p.garageId === garageId);
  const garageObj = garagesDatabase.find((g) => g.id === garageId) || { name: "Connected Garage", phone: "+855 23 888 123" };
  ...
  if (!msgContent) {
    ...
    if (matchTemplate) {
      ...
      if (templateType === "pickup") {
        msgContent = `\u{1F3C1} Vehicle Ready for Pickup!
...
Invoice balance: $45.00. Thank you!`;
        ...
        msgContent = `\u{1F527} Service Status Update:
...
Toyota Tacoma has been placed on the diagnostics bay. Valve adjustments in progress. Expected completion: Today 5:30 PM.`;
        ...
        msgContent = `\u{1F9FE} Tax Invoice Created:
...
Friendly warning that Invoice #INV-2026-112 is still pending clearance. Please settle within 48h to avoid service holds.`;
        ...
        msgContent = `\u{1F4CB} Quotation Approval Request:
...
New service estimate created for your brake overhaul ($75.05). Please click 'Approve' inside the MyCar App before we initiate repairs.`;
        ...
        msgContent = `\u{1F4C5} Next Service Scheduled:
...
app.get("/api/telegram/garage-templates", (req, res) => {
...
app.post("/api/telegram/garage-templates", (req, res) => {
...
app.get("/api/telegram/admin/suspension-status", (req, res) => {
...
app.post("/api/telegram/admin/toggle-suspension", (req, res) => {
...
app.post("/api/notifications/:id/approve", (req, res) => {
  const { id } = req.params;
  const index = notificationLogsDatabase.findIndex((n) => n.id === id);
  if (index !== -1) {
    ...
    if (notif.relatedRecordData && notif.relatedRecordId?.startsWith("req-qr-")) {
      ...
      if (vehicleIdx > -1) {
        ...
        vehicles[vehicleIdx].lastServiceDate = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
      }
...
app.post("/api/notifications/trigger-event", (req, res) => {
  const { eventType, vehicleId, text, customTitle, customMessage } = req.body;
  const vehicle = vehicles.find((v) => v.id === vehicleId) || vehicles[0] || { id: "v1", brand: "Toyota", model: "Tacoma", year: 2006, mileage: 18e4 };
  ...
  switch (eventType) {
    case "maintenance_due":
      title = `\u{1F527} Maintenance Due Soon: ${vehicle.brand} ${vehicle.model}`;
    ...
    case "custom_alarm":
      ...
      sourceType = "user";
      break;
    case "garage_suggested":
      title = `\u{1F6E0}\uFE0F Jet Garage: Post-Service Recommendation`;
      ...
      relatedRecordData = {
        ...
        category: "Engine Oil Change",
        dueDate: new Date(Date.now() + 90 * 24 * 3600 * 1e3).toISOString().split("T")[0],
        dueMileage: (vehicle.mileage || 18e4) + 5e3,
        priority: "Medium",
        description: "Post-service engine health protection suggested by verified technician."
      };
      break;
    case "qr_code_scanned":
      title = `\u{1F4F1} QR Service Record Proposal: Jet Garage`;
    ...
    case "global_risk":
      ...
      break;
    case "recall_alert":
      title = `\u{1F6D1} CRITICAL: Takata Airbag Safety Recall Alert`;
      ...
      break;
    case "booking_update":
      title = `\u{1F4C5} Appointment Confirmed: Premier Angkor Garage`;
      ...
      break;
    case "service_status":
      title = `\u{1F697} Service Status Update: Waiting for Approval`;
    ...
    case "marketplace_offer":
      title = `\u{1F3F7}\uFE0F Parts Marketplace: New Offer Received!`;
      ...
      break;
    case "forum_reply":
      title = `\u{1F4AC} Community Forum: Reply on Jeep 1990 Help Post`;
    ...
    case "admin_broadcast":
      title = title || `\u{1F4E2} City Road Flooding Flash Warning`;
      ...
      break;
    case "weather_alert":
      title = `\u{1F327}\uFE0F Monsoon Safety Alert: Rainy Season Check`;
      ...
      break;
    default:
      title = customTitle || "\u{1F4A1} Alert Center Bulletin";
      category = "maintenance";
      priority = "Low";
      sourceType = "system";
  }
  const newLog = {
    ...
    priority,
    sourceType,
    actionLabel: actionLabel || void 0,
    actionUrl: actionUrl || void 0,
    relatedRecordId: relatedRecordId || void 0,
    relatedRecordData: relatedRecordData || void 0
  };
...
app.post("/api/maintenance", (req, res) => {
  ...
  if (!vehicleId || !serviceCategory || !cost || !mileage) {
  ...
  if (vehicleIdx > -1) {
    ...
    vehicles[vehicleIdx].lastServiceDate = date || (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
  }
  res.json(record);
});
...
app.get("/api/forum/posts", (req, res) => {
  ...
  if (search) {
    ...
    results = results.filter(
      (p) => p.title.toLowerCase().includes(s) || p.description.toLowerCase().includes(s) || p.vehicleModel.toLowerCase().includes(s) || p.vehicleBrand.toLowerCase().includes(s)
    );
  }
  if (category) {
...
app.post("/api/forum/posts", async (req, res) => {
  ...
  const newPost = {
    ...
    mileage: mileage ? Number(mileage) : void 0,
        ...
        config: {
          ...
          responseSchema: {
            ...
            properties: {
              ...
              suggestedCategories: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
              possibleCauses: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
              suggestedChecks: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
              safetyWarning: { type: import_genai.Type.STRING }
            },
  ...
  notificationLogsDatabase.unshift({
    ...
    title: `\u{1F4DD} Help Forum Post Published`,
...
app.post("/api/forum/posts/:id/comments", (req, res) => {
  ...
  const newComment = {
    ...
    authorBadge: authorBadge || (authorRole === "Verified Mechanic" ? "Community Expert" : void 0),
    content,
    timestamp: (/* @__PURE__ */ new Date()).toISOString(),
    upvotes: 0,
  ...
  notificationLogsDatabase.unshift({
    ...
    title: `\u{1F4AC} New Forum Reply on "${post.title.substring(0, 30)}..."`,
...
app.post("/api/forum/posts/:postId/comments/:commentId/upvote", (req, res) => {
...
app.post("/api/forum/posts/:id/accept-solution", (req, res) => {
  const { id } = req.params;
  ...
  if (saveToHistory) {
    const record = {
      id: `m-forum-${Date.now()}`,
      vehicleId: "v1",
      // Link to default user vehicle Tacoma
      ...
      mileage: post.mileage ? Number(post.mileage) : 18e4,
      date: (/* @__PURE__ */ new Date()).toISOString().split("T")[0],
  ...
  notificationLogsDatabase.unshift({
    ...
    title: `\u{1F389} Thread Marked Solved: "${post.title.substring(0, 30)}..."`,
...
app.post("/api/forum/posts/:id/ai-re-diagnose", async (req, res) => {
  const { id } = req.params;
  const post = forumPostsDatabase.find((p) => p.id === id);
  if (!post) {
    return res.status(404).json({ error: "Post not found" });
...
app.get("/api/classifieds/listings", (req, res) => {
  ...
  if (search) {
    const s = String(search).toLowerCase();
    results = results.filter(
      (p) => p.title.toLowerCase().includes(s) || p.description.toLowerCase().includes(s) || p.partNumber && p.partNumber.toLowerCase().includes(s) || p.vehicleModel.toLowerCase().includes(s) || p.vehicleBrand.toLowerCase().includes(s)
    );
  }
  if (category) {
...
app.get("/api/classifieds/listings/:id", (req, res) => {
  const { id } = req.params;
...
app.post("/api/classifieds/listings", async (req, res) => {
  const {
    title,
    description,
    postType,
  ...
  const newListing = {
    ...
    availabilityStatus: "In Stock",
    status: "Active",
    // Live classified immediately, or pending if flagged
    views: 0,
    offerCount: 0,
    createdAt: (/* @__PURE__ */ new Date()).toISOString(),
    isBoosted: false,
    ...
    try {
      ...
      const aiRes = await generateContentWithRetry(client, {
        ...
        config: {
          ...
          responseSchema: {
            type: import_genai.Type.OBJECT,
            properties: {
              aiCompatibilityComment: { type: import_genai.Type.STRING },
              aiSuggestedPriceRange: { type: import_genai.Type.STRING },
      ...
      if (aiRes && aiRes.text) {
        ...
        if (parsed.isSuspicious) {
          newListing.status = "Pending Approval";
        }
      }
    } catch (err) {
  ...
  notificationLogsDatabase.unshift({
    ...
    title: `\u{1F4E6} Part Marketplace Listing Live`,
...
app.post("/api/classifieds/listings/:id/boost", (req, res) => {
  const { id } = req.params;
  const index = classifiedListingsDatabase.findIndex((l) => l.id === id);
  if (index === -1) {
    return res.status(404).json({ error: "Listing not found" });
...
app.post("/api/classifieds/listings/:id/offers", (req, res) => {
  const { id } = req.params;
  ...
  notificationLogsDatabase.unshift({
    ...
    title: `\u{1F91D} New Proposal on "${listing.title.substring(0, 30)}..."`,
...
app.get("/api/classifieds/listings/:id/offers", (req, res) => {
  const { id } = req.params;
  const offers = partOffersDatabase.filter((o) => o.listingId === id);
  res.json(offers);
});
app.post("/api/classifieds/listings/:id/comments", (req, res) => {
  const { id } = req.params;
...
app.post("/api/classifieds/listings/:id/offers/:offerId/status", (req, res) => {
  ...
  if (status === "Accepted") {
    ...
    if (saveToHistory) {
      const record = {
        ...
        cost: serviceCost ? Number(serviceCost) : 15,
        // Simple fitment labor cost
        mileage: 181e3,
        date: (/* @__PURE__ */ new Date()).toISOString().split("T")[0],
    ...
    notificationLogsDatabase.unshift({
      ...
      title: `\u{1F389} Deal Secured on Parts Exchange!`,
...
app.post("/api/classifieds/listings/:id/report", (req, res) => {
  const { id } = req.params;
...
app.get("/api/classifieds/reports", (req, res) => {
  res.json(partReportsDatabase);
});
app.post("/api/classifieds/reports/:reportId/action", (req, res) => {
...
app.post("/api/classifieds/ai-generate", async (req, res) => {
      ...
      const aiRes = await generateContentWithRetry(client, {
        ...
        config: {
          ...
          responseSchema: {
            type: import_genai.Type.OBJECT,
            properties: {
              aiTitle: { type: import_genai.Type.STRING },
              aiDescription: { type: import_genai.Type.STRING }
            },
...
app.post("/api/parts/ai-assist", async (req, res) => {
          ...
          config: {
            ...
            responseSchema: {
              ...
              properties: {
                aiTitle: { type: import_genai.Type.STRING },
                aiDescription: { type: import_genai.Type.STRING },
                aiDescriptionKhmer: { type: import_genai.Type.STRING },
                sellingPoints: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
                compatibleModels: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
                suggestedCategory: { type: import_genai.Type.STRING },
                suggestedPriceRange: { type: import_genai.Type.STRING },
                seoKeywords: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } }
              },
              ...
              properties: {
                restockRecommendations: {
                  ...
                  items: {
                    type: import_genai.Type.OBJECT,
                    properties: {
                      sku: { type: import_genai.Type.STRING },
                      productName: { type: import_genai.Type.STRING },
                      currentStock: { type: import_genai.Type.INTEGER },
                      minStock: { type: import_genai.Type.INTEGER },
                ...
                fastMovingAlerts: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
                slowMovingAlerts: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
                promotionSuggestions: {
                  type: import_genai.Type.ARRAY,
                  items: {
                    type: import_genai.Type.OBJECT,
                    properties: {
                      ...
                      discountPercent: { type: import_genai.Type.NUMBER },
                      reason: { type: import_genai.Type.STRING }
                    },
                    required: ["name", "discountPercent", "reason"]
                  }
                },
                popularCarModelsDemands: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } }
              },
        ...
        const aiRes = await generateContentWithRetry(client, {
          ...
          config: {
            ...
            responseSchema: {
              type: import_genai.Type.OBJECT,
              properties: {
                suggestedParts: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
                compatibilityReason: { type: import_genai.Type.STRING },
                suggestedGarageServices: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
                averagePriceRange: { type: import_genai.Type.STRING },
                matchingInCatalog: {
                  type: import_genai.Type.ARRAY,
                  items: {
                    type: import_genai.Type.OBJECT,
                    properties: {
                      ...
                      stockQuantity: { type: import_genai.Type.INTEGER }
                    },
  ...
  if (taskType === "generate-listing") {
    ...
    const lowStock = items.filter((i) => i.stockQuantity <= i.minStockAlert);
    const restockRecommendations = lowStock.map((i) => ({
      sku: i.sku,
      productName: i.name,
    ...
    let match = inventoryList.find((i) => {
      const nameL = i.name.toLowerCase();
      const probL = problem.toLowerCase();
      return probL.includes("brake") && nameL.includes("brake") || probL.includes("battery") && nameL.includes("battery") || probL.includes("spark") && nameL.includes("spark") || probL.includes("plug") && nameL.includes("plug") || probL.includes("filter") && nameL.includes("filter");
    });
...
app.post("/api/ai/diagnosis", async (req, res) => {
  ...
  const mileageKm = vehicle ? vehicle.mileage : 12e4;
  const client = getGeminiClient();
  if (client) {
    try {
        ...
        config: {
          ...
          responseSchema: {
            ...
            properties: {
              ...
              possible_causes: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
              risk_level: { type: import_genai.Type.STRING, enum: ["low", "medium", "high", "emergency"] },
              safe_user_checks: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
              garage_inspection_needed: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
              recommended_service_category: { type: import_genai.Type.STRING },
              urgency: { type: import_genai.Type.STRING },
              continue_driving_advice: { type: import_genai.Type.STRING, enum: ["safe_to_continue_short_term", "drive_carefully_to_garage", "do_not_drive", "call_emergency_support"] },
              safety_warning: { type: import_genai.Type.STRING },
              confidence_level: { type: import_genai.Type.STRING, enum: ["low", "medium", "high"] }
            },
            required: [
              "summary",
              "possible_causes",
...
app.post("/api/ai/car-advisor", async (req, res) => {
      ...
      const response = await generateContentWithRetry(client, {
        ...
        config: {
          ...
          responseSchema: {
            ...
            properties: {
              summary: { type: import_genai.Type.STRING },
              affordabilityScore: { type: import_genai.Type.INTEGER },
              recommendedBudgetMin: { type: import_genai.Type.INTEGER },
              recommendedBudgetMax: { type: import_genai.Type.INTEGER },
              safeMonthlySpendingLimit: { type: import_genai.Type.INTEGER },
              estimatedMonthlyOwnershipCost: { type: import_genai.Type.INTEGER },
              recommendedCarType: { type: import_genai.Type.STRING },
              options: {
                type: import_genai.Type.ARRAY,
                items: {
                  type: import_genai.Type.OBJECT,
                  properties: {
                    ...
                    yearRange: { type: import_genai.Type.STRING },
                    estimatedPrice: { type: import_genai.Type.STRING },
                    fuelEfficiency: { type: import_genai.Type.STRING },
                    fitExplanation: { type: import_genai.Type.STRING },
                    riskRating: { type: import_genai.Type.STRING },
                    maintenanceCostRating: { type: import_genai.Type.STRING },
                    sparePartAvailability: { type: import_genai.Type.STRING }
                  },
              ...
              newVsUsedRecommendation: { type: import_genai.Type.STRING },
              fuelTypeRecommendation: { type: import_genai.Type.STRING },
              longTermOwnershipAdvice: { type: import_genai.Type.STRING },
              maintenanceCostWarning: { type: import_genai.Type.STRING },
              buyingTimingSuggestion: { type: import_genai.Type.STRING },
              prePurchaseInspectionChecklist: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
              finalRecommendation: { type: import_genai.Type.STRING },
              disclaimer: { type: import_genai.Type.STRING }
            },
            required: [
              "summary",
              "affordabilityScore",
              "recommendedBudgetMin",
              "recommendedBudgetMax",
              "safeMonthlySpendingLimit",
              "estimatedMonthlyOwnershipCost",
              "recommendedCarType",
              "options",
              "newVsUsedRecommendation",
              "fuelTypeRecommendation",
              "longTermOwnershipAdvice",
              "maintenanceCostWarning",
              "buyingTimingSuggestion",
      ...
      const response = await generateContentWithRetry(client, {
        ...
        config: {
          ...
          responseSchema: {
            ...
            properties: {
              summary: { type: import_genai.Type.STRING },
              targetCount: { type: import_genai.Type.INTEGER },
              campaignName: { type: import_genai.Type.STRING },
              suggestedMessage: { type: import_genai.Type.STRING },
              partnerOffer: { type: import_genai.Type.STRING },
              actionableSteps: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } }
            },
...
function generateSimulatedReport(brand, model, year) {
...
function generateSimulatedAdvisorReport(profile, usage, preference) {
  ...
  const savings = parseFloat(profile.current_savings) || 3e3;
  const downPayment = parseFloat(profile.down_payment) || 2e3;
  const isCash = (profile.buying_method || "").toLowerCase().includes("cash");
  const monthlyNet = salary + extra - expense;
  let score = 70;
  if (savings < 3e3) score -= 15;
  if (monthlyNet < 300) score -= 20;
  if (monthlyNet > 1e3) score += 15;
  ...
  let budgetMax = 18e3;
  if (isCash) {
    budgetMin = Math.max(3e3, downPayment * 0.8);
    budgetMax = Math.max(5e3, downPayment + savings * 0.5);
  } else {
    budgetMin = Math.max(5e3, downPayment + monthlyNet * 15);
    budgetMax = Math.max(1e4, downPayment + monthlyNet * 36);
  }
...
function generateSimulatedDiagnosis(symptom, brandStr) {
...
function generateSimulatedAdminQuery(question, overdueCount, totalCount) {
